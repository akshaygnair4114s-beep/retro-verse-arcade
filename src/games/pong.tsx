...
export default function Pong() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent, down: boolean) => {
      if (e.key === "ArrowUp" || e.key === "w") { state.up = down; e.preventDefault(); }
      if (e.key === "ArrowDown" || e.key === "s") { state.down = down; e.preventDefault(); }
    };
    ...
    const resetBall = (dir: number) => {
      ...
      state.vx = 5 * dir; state.vy = (Math.random() * 4 - 2);
    };
    ...
    const loop = () => {
      ...
      // collisions
      if (state.bx < 20 + PADDLE_W && state.by + BALL > state.py && state.by < state.py + PADDLE_H && state.vx < 0) {
        state.vx *= -1.05; state.vx = Math.min(state.vx, 12);
        const hit = (state.by - (state.py + PADDLE_H / 2)) / (PADDLE_H / 2);
        state.vy = hit * 6;
      }
      if (state.bx > W - 20 - PADDLE_W - BALL && state.by + BALL > state.ay && state.by < state.ay + PADDLE_H && state.vx > 0) {
        state.vx *= -1.05; state.vx = Math.max(state.vx, -12);
        const hit = (state.by - (state.ay + PADDLE_H / 2)) / (PADDLE_H / 2);
        state.vy = hit * 6;
      }
      if (state.bx < 0) { setScore((s) => ({ ...s, ai: s.ai + 1 })); resetBall(1); }
      if (state.bx > W) { setScore((s) => { const ns = { ...s, p: s.p + 1 }; submit(ns.p); return ns; }); resetBall(-1); }
      ...
      // ball
      ctx.shadowColor = "#FFE600"; ctx.fillStyle = "#FFE600";
