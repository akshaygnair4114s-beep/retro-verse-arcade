...
function beep(type: "dice" | "move" | "ladder" | "snake" | "win") {
  try {
    ...
    if (type === "dice")   for (let i = 0; i < 8; i++) tone(120 + Math.random() * 280, i * 0.065, 0.06, "square");
    if (type === "move")   tone(440, 0, 0.06);
    if (type === "ladder") [330, 440, 554, 659].forEach((f, i) => tone(f, i * 0.1, 0.15));
    if (type === "snake")  [659, 554, 440, 330].forEach((f, i) => tone(f, i * 0.1, 0.12, "sawtooth"));
...
function DiceFace({ val, rolling }: { val: number; rolling: boolean }) {
  ...
  return (
    <div
      ...
      style={{
        boxShadow: rolling ? "0 0 28px oklch(0.84 0.18 215 / 0.85)" : "0 0 10px oklch(0.84 0.18 215 / 0.3)",
          ...
          <div key={k} className="grid place-items-center">
            {filled && (
              <div
                ...
                style={{ width: "clamp(5px, 1.4vw, 10px)", height: "clamp(5px, 1.4vw, 10px)", boxShadow: "0 0 4px oklch(0.84 0.18 215)" }}
              />
            )}
          </div>
        );
...
function Board({ players }: { players: Player[] }) {
  ...
  Object.entries(LADDERS).forEach(([from, to]) => {
    const a = squareSVG(+from), b = squareSVG(to);
    const dx = b.cx - a.cx, dy = b.cy - a.cy;
    const len = Math.hypot(dx, dy) || 1;
    const nx = (-dy / len) * 1.4, ny = (dx / len) * 1.4;
    svgEls.push(
      <g key={`L${from}`} filter="url(#glow-g)" opacity="0.9">
        <line x1={a.cx+nx} y1={a.cy+ny} x2={b.cx+nx} y2={b.cy+ny} stroke="#22ff66" strokeWidth="0.65"/>
        <line x1={a.cx-nx} y1={a.cy-ny} x2={b.cx-nx} y2={b.cy-ny} stroke="#22ff66" strokeWidth="0.65"/>
        {[0.25, 0.5, 0.75].map((t, i) => (
          <line key={i}
            x1={a.cx+dx*t - nx*1.4} y1={a.cy+dy*t - ny*1.4}
            x2={a.cx+dx*t + nx*1.4} y2={a.cy+dy*t + ny*1.4}
            stroke="#22ff66" strokeWidth="0.5"
          />
        ))}
      </g>
    );
  ...
  Object.entries(SNAKES).forEach(([from, to]) => {
    ...
    svgEls.push(
      <path key={`S${from}`}
        ...
        fill="none" stroke="#ff2244" strokeWidth="1.4" strokeLinecap="round"
        filter="url(#glow-r)" opacity="0.95"
      />
    );
  });

  const cells = Array.from({ length: 100 }, (_, idx) => {
    const gx = idx % 10, gy = Math.floor(idx / 10);
    const sq = cellToSquare(gx, gy);
    const isLBase = sq in LADDERS, isLTop  = Object.values(LADDERS).includes(sq);
    const isSHead = sq in SNAKES,  isST    = Object.values(SNAKES).includes(sq);
    const here = posMap.get(sq) ?? [], isFin = sq === 100;

    const bg = isFin ? "#1f0a35"
      : isLBase ? "#002900" : isLTop  ? "#001900"
      : isSHead ? "#2a0000" : isST    ? "#180000"
      : (gy + gx) % 2 === 0 ? "#100e22" : "#140c28";

    return (
      <div key={idx} className="relative" style={{ backgroundColor: bg, border: "1px solid rgba(255,255,255,0.04)" }}>
        {/* Square number */}
        <span
          className={`absolute top-px left-px font-mono leading-none select-none pointer-events-none ${
            isFin ? "text-neon-yellow font-bold"
            : isLBase || isLTop ? "text-green-400/70"
            : isSHead || isST ? "text-red-400/70"
            : "text-white/20"
          }`}
        ...
        {/* Emoji markers */}
        {isLBase && <span className="absolute bottom-px right-px pointer-events-none" style={{ fontSize: "clamp(5px, 1vw, 11px)" }}>🪜</span>}
        {isSHead && <span className="absolute bottom-px right-px pointer-events-none" style={{ fontSize: "clamp(5px, 1vw, 11px)" }}>🐍</span>}
        {isFin   && <span className="absolute inset-0 grid place-items-center pointer-events-none" style={{ fontSize: "clamp(9px, 1.8vw, 22px)" }}>🏆</span>}
  ...
  return (
    ...
    >
      <div className="grid grid-cols-10 w-full h-full" style={{ gridTemplateRows: "repeat(10, 1fr)" }}>
        {cells}
      </div>
      <svg className="absolute inset-0 pointer-events-none w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
        <defs>
          <filter id="glow-g"><feGaussianBlur stdDeviation="0.35" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <filter id="glow-r"><feGaussianBlur stdDeviation="0.35" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        </defs>
        {svgEls}
      </svg>
    </div>
...
// ── Rules screen ─────────────────────────────────────────────────────────
function RulesScreen({ onPlay, onBack, onClose }: { onPlay?: () => void; onBack?: () => void; onClose?: () => void }) {
  const RULES = [
    { icon: "🎯", t: "Objective",        d: "Be the first player to land exactly on square 100 to win!" },
    { icon: "🎲", t: "Your Turn",        d: "Press Roll Dice. Your token moves forward by the number rolled (1–6). Players alternate turns." },
    { icon: "🪜", t: "Ladders — Up!",    d: "Land on the bottom of a ladder and you instantly climb to the top — a big shortcut!" },
    { icon: "🐍", t: "Snakes — Down!",   d: "Land on a snake's head and you slide all the way down to its tail. Watch out!" },
    { icon: "🏆", t: "Winning",          d: "You must land EXACTLY on square 100. If the roll would take you past 100, you stay in place." },
  ];
  return (
    <div className="p-4 sm:p-6 max-w-xl mx-auto">
      <div className="font-mono text-xs uppercase tracking-[0.4em] text-neon-yellow text-center">How to play</div>
      <h2 className="mt-1 font-display text-2xl font-black neon-text-cyan text-center">Rules</h2>
      <div className="mt-5 space-y-3">
        {RULES.map((r) => (
          <div key={r.t} className="glass rounded-xl p-4 ring-1 ring-white/10 flex gap-3 items-start">
      ...
      <div className="mt-5 flex flex-wrap gap-2 justify-center">
        {onPlay  && <button onClick={onPlay}  className="btn-neon">▶ Start Game</button>}
        {onBack  && <button onClick={onBack}  className="btn-ghost-neon">← Back</button>}
        {onClose && <button onClick={onClose} className="btn-ghost-neon">Close</button>}
      </div>
    </div>
  );
}
...
function SetupScreen({ onNext }: { onNext: (players: Player[]) => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onNext(Array.from({ length: count }, (_, i) => ({
  ...
  return (
    <div className="flex flex-col items-center py-6 px-4 min-h-[420px]">
      <div className="w-full max-w-lg">
        <div className="font-mono text-xs uppercase tracking-[0.4em] text-neon-yellow text-center">Multiplayer · 2–10P</div>
        <h2 className="mt-1 font-display text-2xl sm:text-3xl font-black neon-text-cyan text-center">Snakes &amp; Ladders</h2>
        <p className="mt-2 text-sm text-muted-foreground text-center">Classic board game for 2–10 players.</p>

        <form onSubmit={submit} className="mt-6 space-y-5">
          <div>
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">Number of players</div>
          ...
          <div>
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">Player names</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {Array.from({ length: count }, (_, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span
                    className="w-3 h-3 rounded-full flex-shrink-0 ring-1 ring-white/20"
                    style={{ backgroundColor: COLORS[i].hex, boxShadow: `0 0 6px ${COLORS[i].glow}` }}
                  />
                  <input
                    value={names[i]}
          ...
          <button type="submit" className="btn-neon w-full">View Rules &amp; Play →</button>
        </form>
      </div>
    </div>
  );
...
function GameBoard({ initial, onNewGame }: { initial: Player[]; onNewGame: () => void }) {
      ...
      stepRef.current = setInterval(() => {
        ...
        delay(() => {
          ...
          if (LADDERS[target]) {
            ...
            beep("ladder");
            setFlash({ msg: `🪜 ${player.name} climbed a ladder! ${target} → ${final}`, cls: "ladder" });
  ...
  const restart = useCallback(() => {
    ...
    setCur(0); setDiceVal(1); setRolling(false);
    setBusy(false); setWinner(null); setLog([]); setFlash(null);
  }, [initial]);
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_248px] items-start">
        ...
        <div className="relative">
          ...
          {flash && (
            <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none rounded-xl">
              <div className={`font-display font-black text-sm sm:text-lg text-center px-4 py-2 rounded-xl glass ring-1 mx-3 ${
                flash.cls === "ladder" ? "ring-neon-green text-neon-green" : "ring-neon-magenta text-neon-magenta"
              }`}>
                {flash.msg}
              </div>
            </div>
          ...
          {winner !== null && (
            <div className="absolute inset-0 z-20 grid place-items-center bg-black/88 rounded-xl backdrop-blur-sm">
              <div className="text-center px-6">
                <div className="font-mono text-[10px] uppercase tracking-[0.4em] text-neon-yellow">Champion!</div>
                <div className="mt-2 font-display text-3xl sm:text-4xl font-black neon-text-cyan">
                  {players[winner].name}
                </div>
                <div className="mt-1 text-sm text-muted-foreground font-mono">Reached square 100! 🏆</div>
                <div className="mt-5 flex flex-wrap gap-2 justify-center">
                  <button className="btn-neon" onClick={onNewGame}>New Game</button>
                  <button className="btn-ghost-neon" onClick={restart}>Play Again</button>
                </div>
              </div>
            </div>
          )}
        ...
        <aside className="grid gap-3 grid-cols-2 lg:grid-cols-1">
          ...
          <div className="glass rounded-xl p-3 ring-1 ring-white/10">
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Current Turn</div>
            <div className="mt-1.5 flex items-center gap-2">
              <span
                className="w-3.5 h-3.5 rounded-full flex-shrink-0"
                style={{ backgroundColor: COLORS[cp.colorIdx].hex, boxShadow: `0 0 8px ${COLORS[cp.colorIdx].glow}` }}
              />
              <span className="font-display font-bold text-base neon-text-cyan truncate">{cp.name}</span>
            </div>
          ...
          <div className="glass rounded-xl p-3 ring-1 ring-white/10 col-span-2 lg:col-span-1">
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">Players</div>
              ...
              {players.map((p, i) => (
                ...
                >
                  <span
                    ...
                    style={{ backgroundColor: COLORS[p.colorIdx].hex, boxShadow: `0 0 4px ${COLORS[p.colorIdx].glow}` }}
                  />
                  <span className="font-mono truncate flex-1 text-foreground/90">{p.name}</span>
                  <span className="font-mono tabular-nums text-neon-cyan text-[10px]">{p.pos || "—"}</span>
                  {winner === i && <span>👑</span>}
                </div>
              ))}
            </div>
          ...
          {/* Game log */}
          <div className="glass rounded-xl p-3 ring-1 ring-white/10 col-span-2 lg:col-span-1">
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5">Game Log</div>
            <div className="space-y-0.5 max-h-28 sm:max-h-36 overflow-y-auto">
              {log.length === 0 ? (
                <div className="text-[10px] text-muted-foreground font-mono italic">No moves yet…</div>
              ) : log.map((l) => (
                <div key={l.id} className={`text-[10px] font-mono leading-snug ${
                  l.cls === "ladder" ? "text-neon-green" :
                  l.cls === "snake"  ? "text-neon-magenta" :
                  l.cls === "win"    ? "text-neon-yellow" :
          ...
          {/* Action buttons */}
          <button onClick={() => setShowRules(true)} className="btn-ghost-neon !text-xs">📖 Rules</button>
          <button onClick={restart}   className="btn-ghost-neon !text-xs">↺ Restart</button>
          <button onClick={onNewGame} className="btn-ghost-neon !text-xs col-span-2 lg:col-span-1">✦ New Game</button>
        </aside>
      </div>
    </div>
  );
...
export default function SnakesAndLadders() {
  ...
  if (phase === "setup") {
    return <SetupScreen onNext={(ps) => { setPlayers(ps); setPhase("rules"); }} />;
  }
  if (phase === "rules") {
