...
function slideRow(row: number[]): { row: number[]; gained: number } {
  ...
  for (let i = 0; i < filtered.length - 1; i++) {
    if (filtered[i] === filtered[i + 1]) {
      filtered[i] *= 2; gained += filtered[i]; filtered.splice(i + 1, 1);
    }
  }
...
export default function G2048() {
  const [saved, setSaved, loaded] = usePersistentState<{ g: Grid; s: number } | null>("g2048", null);
  ...
  useEffect(() => {
    ...
    if (saved && saved.g.flat().some((v) => v)) {
      setGrid(saved.g); setScore(saved.s);
    } else {
  ...
  const doMove = useCallback((dir: "L" | "R" | "U" | "D") => {
    ...
    setGrid(withNew);
    setScore((s) => { const n = s + gained; submit(n); return n; });
