...
const accentMap: Record<GameMeta["accent"], { ring: string; glow: string; text: string }> = {
  cyan:    { ring: "ring-neon-cyan/40",    glow: "shadow-[0_0_30px_oklch(0.84_0.18_215/0.35)]", text: "text-neon-cyan" },
  magenta: { ring: "ring-neon-magenta/40", glow: "shadow-[0_0_30px_oklch(0.7_0.28_348/0.35)]",  text: "text-neon-magenta" },
  yellow:  { ring: "ring-neon-yellow/40",  glow: "shadow-[0_0_30px_oklch(0.92_0.18_100/0.35)]", text: "text-neon-yellow" },
  green:   { ring: "ring-neon-green/40",   glow: "shadow-[0_0_30px_oklch(0.85_0.22_145/0.35)]", text: "text-neon-green" },
  purple:  { ring: "ring-neon-purple/40",  glow: "shadow-[0_0_30px_oklch(0.65_0.24_300/0.35)]", text: "text-neon-purple" },
};

export function GameCard({ game }: { game: GameMeta }) {
  const a = accentMap[game.accent];
  const inner = (
    <div className={`group relative h-full overflow-hidden rounded-xl glass ring-1 ${a.ring} transition-all duration-300 hover:-translate-y-1 hover:${a.glow}`}>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono uppercase text-muted-foreground tracking-widest">{game.category}</span>
        </div>
